...
final Provider<FirestoreSleepService> firestoreSleepServiceProvider =
    Provider<FirestoreSleepService>((ProviderRef<FirestoreSleepService> ref) {
  return FirestoreSleepService();
});

// Sleep Sessions Provider with Real-time Firebase Stream
final StreamProvider<List<SleepSession>> sleepSessionsStreamProvider =
    StreamProvider<List<SleepSession>>(
        (StreamProviderRef<List<SleepSession>> ref) {
  final FirestoreSleepService sleepService =
      ref.watch(firestoreSleepServiceProvider);
  return sleepService.watchSleepSessions();
});

// Sleep Sessions State Notifier for write operations
final StateNotifierProvider<SleepSessionsNotifier,
        AsyncValue<List<SleepSession>>> sleepSessionsProvider =
    StateNotifierProvider<SleepSessionsNotifier,
        AsyncValue<List<SleepSession>>>((StateNotifierProviderRef<
            SleepSessionsNotifier, AsyncValue<List<SleepSession>>>
        ref) {
  return SleepSessionsNotifier(ref.watch(firestoreSleepServiceProvider), ref);
});

class SleepSessionsNotifier
    extends StateNotifier<AsyncValue<List<SleepSession>>> {
  final FirestoreSleepService _sleepService;
  final StateNotifierProviderRef<SleepSessionsNotifier,
      AsyncValue<List<SleepSession>>> _ref;

  SleepSessionsNotifier(this._sleepService, this._ref)
      : super(const AsyncValue.loading()) {
    // Listen to the stream provider for real-time updates
    _ref.listen<AsyncValue<List<SleepSession>>>(sleepSessionsStreamProvider,
        (AsyncValue<List<SleepSession>>? previous,
            AsyncValue<List<SleepSession>> next) {
      state = next;
    });
  }
...
final Provider<AsyncValue<SleepSession?>> latestSleepSessionProvider =
    Provider<AsyncValue<SleepSession?>>(
        (ProviderRef<AsyncValue<SleepSession?>> ref) {
  final AsyncValue<List<SleepSession>> sessionsAsync =
      ref.watch(sleepSessionsStreamProvider);
  return sessionsAsync.when(
    data: (List<SleepSession> sessions) =>
        AsyncValue.data(sessions.isNotEmpty ? sessions.first : null),
...
final FutureProvider<SleepStats> sleepStatsProvider =
    FutureProvider<SleepStats>((FutureProviderRef<SleepStats> ref) async {
  ...
  try {
    ...
    print(
        '📅 Loading stats for date range: ${startDate.toIso8601String().split('T')[0]} to ${endDate.toIso8601String().split('T')[0]}');
    ...
    final SleepStats stats = SleepStats(
      averageDuration: Duration(
          minutes:
              ((analytics['averageDuration'] as num).toDouble() * 60).round()),
      averageQuality: (analytics['averageQuality'] as num).toDouble(),
      totalSleepTime: Duration(
          minutes:
              ((analytics['totalSleepTime'] as num).toDouble() * 60).round()),
      totalSessions: analytics['totalSessions'] as int,
      bestSleep:
          Duration(hours: 8), // You may want to calculate this from sessions
      worstSleep:
          Duration(hours: 4), // You may want to calculate this from sessions
      ...
      weeklyTrends: (analytics['moodBreakdown'] as Map<String, dynamic>).map(
          (String key, dynamic value) =>
              MapEntry<String, double>(key, (value as num).toDouble())),
    );

    print(
        '✅ Sleep stats calculated successfully: ${stats.totalSessions} sessions, avg duration: ${stats.averageDuration.inHours}h${stats.averageDuration.inMinutes % 60}m');
    return stats;
  } catch (error) {
...
final StreamProvider<List<SleepGoal>> sleepGoalsStreamProvider =
    StreamProvider<List<SleepGoal>>((StreamProviderRef<List<SleepGoal>> ref) {
  final FirestoreSleepService sleepService =
      ref.watch(firestoreSleepServiceProvider);
  return sleepService.watchSleepGoals();
});

final StateNotifierProvider<SleepGoalsNotifier, AsyncValue<List<SleepGoal>>>
    sleepGoalsProvider =
    StateNotifierProvider<SleepGoalsNotifier, AsyncValue<List<SleepGoal>>>(
        (StateNotifierProviderRef<SleepGoalsNotifier,
                AsyncValue<List<SleepGoal>>>
            ref) {
...
class SleepGoalsNotifier extends StateNotifier<AsyncValue<List<SleepGoal>>> {
  final FirestoreSleepService _sleepService;
  final StateNotifierProviderRef<SleepGoalsNotifier,
      AsyncValue<List<SleepGoal>>> _ref;

  SleepGoalsNotifier(this._sleepService, this._ref)
      : super(const AsyncValue.loading()) {
    // Listen to the stream provider for real-time updates
    _ref.listen<AsyncValue<List<SleepGoal>>>(sleepGoalsStreamProvider,
        (AsyncValue<List<SleepGoal>>? previous,
            AsyncValue<List<SleepGoal>> next) {
      state = next;
    });
  }
...
final StreamProvider<List<SleepReminder>> sleepRemindersStreamProvider =
    StreamProvider<List<SleepReminder>>(
        (StreamProviderRef<List<SleepReminder>> ref) {
  final FirestoreSleepService sleepService =
      ref.watch(firestoreSleepServiceProvider);
  return sleepService.watchSleepReminders();
});

final StateNotifierProvider<SleepRemindersNotifier,
        AsyncValue<List<SleepReminder>>> sleepRemindersProvider =
    StateNotifierProvider<SleepRemindersNotifier,
        AsyncValue<List<SleepReminder>>>((StateNotifierProviderRef<
            SleepRemindersNotifier, AsyncValue<List<SleepReminder>>>
        ref) {
  return SleepRemindersNotifier(ref.watch(firestoreSleepServiceProvider), ref);
});

class SleepRemindersNotifier
    extends StateNotifier<AsyncValue<List<SleepReminder>>> {
  final FirestoreSleepService _sleepService;
  final StateNotifierProviderRef<SleepRemindersNotifier,
      AsyncValue<List<SleepReminder>>> _ref;

  SleepRemindersNotifier(this._sleepService, this._ref)
      : super(const AsyncValue.loading()) {
    // Listen to the stream provider for real-time updates
    _ref.listen<AsyncValue<List<SleepReminder>>>(sleepRemindersStreamProvider,
        (AsyncValue<List<SleepReminder>>? previous,
            AsyncValue<List<SleepReminder>> next) {
      state = next;
    });
  }
...
final StreamProvider<List<SleepInsight>> sleepInsightsStreamProvider =
    StreamProvider<List<SleepInsight>>(
        (StreamProviderRef<List<SleepInsight>> ref) {
  final FirestoreSleepService sleepService =
      ref.watch(firestoreSleepServiceProvider);
  return sleepService.watchSleepInsights();
});

final StateNotifierProvider<SleepInsightsNotifier,
        AsyncValue<List<SleepInsight>>> sleepInsightsProvider =
    StateNotifierProvider<SleepInsightsNotifier,
        AsyncValue<List<SleepInsight>>>((StateNotifierProviderRef<
            SleepInsightsNotifier, AsyncValue<List<SleepInsight>>>
        ref) {
  return SleepInsightsNotifier(ref.watch(firestoreSleepServiceProvider), ref);
});

class SleepInsightsNotifier
    extends StateNotifier<AsyncValue<List<SleepInsight>>> {
  final FirestoreSleepService _sleepService;
  final StateNotifierProviderRef<SleepInsightsNotifier,
      AsyncValue<List<SleepInsight>>> _ref;

  SleepInsightsNotifier(this._sleepService, this._ref)
      : super(const AsyncValue.loading()) {
    // Listen to the stream provider for real-time updates
    _ref.listen<AsyncValue<List<SleepInsight>>>(sleepInsightsStreamProvider,
        (AsyncValue<List<SleepInsight>>? previous,
            AsyncValue<List<SleepInsight>> next) {
      state = next;
    });
    ...
    try {
      ...
      if (sessions.isEmpty) {
        final SleepInsight welcomeInsight = SleepInsight(
          ...
          description:
              'Begin your sleep journey by tracking your first sleep session.',
        ...
        final double avgQuality = sessions.fold<double>(
                0,
                (double sum, SleepSession session) =>
                    sum + session.sleepQuality) /
            sessions.length;
        final Duration avgDuration = Duration(
          milliseconds: sessions.fold<int>(
                  0,
                  (int sum, SleepSession session) =>
                      sum + session.totalDuration.inMilliseconds) ~/
              sessions.length,
        );
        ...
        if (avgQuality < 7.0) {
          final SleepInsight qualityInsight = SleepInsight(
            ...
            description:
                'Your average sleep quality is below optimal levels (${avgQuality.toStringAsFixed(1)}/10).',
            type: SleepInsightType.quality,
            impact: 0.8,
            recommendations: <String>[
        ...
        if (avgDuration.inHours < 7) {
          final SleepInsight durationInsight = SleepInsight(
            ...
            description:
                'You\'re averaging ${avgDuration.inHours}h ${avgDuration.inMinutes % 60}m, less than the recommended 7-9 hours.',
  ...
  Future<void> refreshInsights() async {
    ...
    final AsyncValue<List<SleepSession>> sessionsAsync =
        _ref.read(sleepSessionsStreamProvider);
...
final StateNotifierProvider<SleepTrackingStateNotifier, SleepTrackingState>
    sleepTrackingStateProvider =
    StateNotifierProvider<SleepTrackingStateNotifier, SleepTrackingState>(
        (StateNotifierProviderRef<SleepTrackingStateNotifier,
                SleepTrackingState>
            ref) {
  return SleepTrackingStateNotifier();
});

enum SleepTrackingState { notTracking, tracking, paused, completed }
...
final StateNotifierProvider<SleepSessionCreationNotifier,
        AsyncValue<SleepSession?>> sleepSessionCreationProvider =
    StateNotifierProvider<SleepSessionCreationNotifier,
        AsyncValue<SleepSession?>>((StateNotifierProviderRef<
            SleepSessionCreationNotifier, AsyncValue<SleepSession?>>
        ref) {
  return SleepSessionCreationNotifier(
      ref.watch(firestoreSleepServiceProvider), ref);
});

class SleepSessionCreationNotifier
    extends StateNotifier<AsyncValue<SleepSession?>> {
  final FirestoreSleepService _sleepService;
  final StateNotifierProviderRef<SleepSessionCreationNotifier,
      AsyncValue<SleepSession?>> _ref;

  SleepSessionCreationNotifier(this._sleepService, this._ref)
...
final Provider<AsyncValue<Map<String, double>>> weeklySleepTrendsProvider =
    Provider<AsyncValue<Map<String, double>>>(
        (ProviderRef<AsyncValue<Map<String, double>>> ref) {
...
final Provider<AsyncValue<double>> sleepQualityScoreProvider =
    Provider<AsyncValue<double>>((ProviderRef<AsyncValue<double>> ref) {
  final AsyncValue<SleepStats> statsAsync = ref.watch(sleepStatsProvider);
  return statsAsync.when(
...
final Provider<AsyncValue<Duration>> sleepDurationProvider =
    Provider<AsyncValue<Duration>>((ProviderRef<AsyncValue<Duration>> ref) {
  final AsyncValue<SleepStats> statsAsync = ref.watch(sleepStatsProvider);
  return statsAsync.when(
...
final Provider<AsyncValue<double>> sleepConsistencyScoreProvider =
    Provider<AsyncValue<double>>((ProviderRef<AsyncValue<double>> ref) {
  final AsyncValue<SleepStats> statsAsync = ref.watch(sleepStatsProvider);
  return statsAsync.when(
...
final Provider<AsyncValue<double>> sleepSustainabilityScoreProvider =
    Provider<AsyncValue<double>>((ProviderRef<AsyncValue<double>> ref) {
  final AsyncValue<SleepStats> statsAsync = ref.watch(sleepStatsProvider);
  return statsAsync.when(
...
final Provider<AsyncValue<List<SleepSession>>> todaySleepSessionsProvider =
    Provider<AsyncValue<List<SleepSession>>>(
        (ProviderRef<AsyncValue<List<SleepSession>>> ref) {
  final AsyncValue<List<SleepSession>> sessionsAsync =
      ref.watch(sleepSessionsStreamProvider);
...
final Provider<AsyncValue<List<SleepGoal>>> activeSleepGoalsProvider =
    Provider<AsyncValue<List<SleepGoal>>>(
        (ProviderRef<AsyncValue<List<SleepGoal>>> ref) {
  final AsyncValue<List<SleepGoal>> goalsAsync =
      ref.watch(sleepGoalsStreamProvider);

  return goalsAsync.when(
    data: (List<SleepGoal> goals) {
      final List<SleepGoal> activeGoals =
          goals.where((SleepGoal goal) => goal.reminderEnabled).toList();
...
final Provider<AsyncValue<List<SleepReminder>>> enabledSleepRemindersProvider =
    Provider<AsyncValue<List<SleepReminder>>>(
        (ProviderRef<AsyncValue<List<SleepReminder>>> ref) {
  final AsyncValue<List<SleepReminder>> remindersAsync =
      ref.watch(sleepRemindersStreamProvider);

  return remindersAsync.when(
    data: (List<SleepReminder> reminders) {
      final List<SleepReminder> enabledReminders = reminders
          .where((SleepReminder reminder) => reminder.enabled)
          .toList();
