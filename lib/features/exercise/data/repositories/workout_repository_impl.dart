...
class WorkoutRepositoryImpl implements WorkoutRepository {
  ...
  Future<void> saveWorkout(WorkoutPlan workout) async {
    try {
      ...
      final savedWorkouts = await prefs.getStringList('saved_workouts') ?? [];
  ...
  Future<List<WorkoutPlan>> getSavedWorkouts() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final savedWorkouts = prefs.getStringList('saved_workouts') ?? [];

      return savedWorkouts.map((workoutString) {
        final workoutData = jsonDecode(workoutString) as Map<String, dynamic>;
  ...
  Future<void> deleteWorkout(String workoutId) async {
    try {
      ...
      savedWorkouts.removeWhere((workoutString) {
        final workoutData = jsonDecode(workoutString) as Map<String, dynamic>;
