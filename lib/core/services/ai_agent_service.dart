...
class AIAgentService {
  ...
  final FirestoreNutritionService _nutritionService =
      FirestoreNutritionService();
  ...
  Future<AIAgentResponse> sendMessage(String userMessage) async {
    try {
      ...
      if (e.toString().contains('Connection refused') ||
        ...
        print(
            'AI Agent API server not available, using mock response for testing');
  ...
  bool get hasLastPlan =>
      _lastResponse?.data != null &&
      (_isWorkoutPlan(_lastResponse!.data!) ||
          _isMealPlan(_lastResponse!.data!));

  /// Check if the last response is a workout plan
  bool get isLastResponseWorkout =>
      _lastResponse?.data != null && _isWorkoutPlan(_lastResponse!.data!);

  /// Check if the last response is a meal plan
  bool get isLastResponseMeal =>
      _lastResponse?.data != null && _isMealPlan(_lastResponse!.data!);

  /// Check if the last response is food logging (already saved automatically)
  bool get isLastResponseFoodLogging =>
      _lastResponse?.data != null && _isFoodLogging(_lastResponse!.data!);
  ...
  Future<void> saveWorkoutPlan(Map<String, dynamic> data) async {
    try {
      ...
      print(
          'AIAgentService: Using FirebaseWorkoutService to save to workout_plans collection');

      // Transform the API response to match our WorkoutPlan model
      final Map<String, dynamic> transformedData =
          _transformWorkoutPlanData(data);
      print(
          'AIAgentService: Transformed data structure: ${transformedData.keys.toList()}');

      final WorkoutPlan workoutPlan = WorkoutPlan.fromJson(transformedData);
      final String planName =
          'AI Generated Workout - ${DateTime.now().toLocal().toString().split(' ')[0]}';

      print('AIAgentService: Created WorkoutPlan object successfully');
      print('AIAgentService: Plan name: $planName');
      print(
          'AIAgentService: Calling FirestoreWorkoutService.saveWorkoutPlan()...');
      ...
      final String savedId =
          await _workoutService.saveWorkoutPlan(savedWorkoutPlan);

      print(
          'AIAgentService: Workout plan saved successfully with ID: $savedId');
      print(
          'AIAgentService: Plan saved to Firestore path: users/$_userId/exercise/data/workout_plans');
    } catch (e) {
  ...
  Map<String, dynamic> _transformWorkoutPlanData(Map<String, dynamic> apiData) {
    ...
    print(
        'AIAgentService: Original sessions count: ${(apiData['workout_sessions'] as List).length}');
    ...
    final List<Map<String, dynamic>> transformedSessions =
        (apiData['workout_sessions'] as List)
      ...
      print(
          'AIAgentService: Session has ${transformedExercises.length} exercises');
      return <String, dynamic>{
    ...
    print(
        'AIAgentService: Transformation complete. Transformed sessions count: ${transformedSessions.length}');
    return result;
  }
  ...
  Future<void> _saveFoodLogEntry(Map<String, dynamic> data) async {
    try {
      ...
      final Map<String, dynamic> macros =
          data['macronutrients'] as Map<String, dynamic>;
      ...
      print(
          'AIAgentService: Food log entry saved successfully with ID: $entryId');
      print(
          'AIAgentService: Entry saved to path: users/$_userId/nutrition/data/food_log_entries');
    } catch (e) {
  ...
  AIAgentResponse _getMockResponse(String userMessage) {
    ...
    if (lowerMessage.contains('workout') || lowerMessage.contains('exercise')) {
      ...
      return AIAgentResponse(
        text:
            'I\'ve created a personalized workout plan for you! This includes strength training and cardio exercises tailored to your fitness level.',
        data: _getMockWorkoutPlan(),
      );
    } else if (lowerMessage.contains('meal') ||
        lowerMessage.contains('nutrition') ||
        lowerMessage.contains('diet')) {
      // Return a mock meal plan
      return AIAgentResponse(
        text:
            'Here\'s a personalized 3-day meal plan designed to meet your nutritional goals with sustainable and healthy options!',
  ...
  String _generateAnalysisResponse(String message) {
    ...
    } else if (lowerMessage.contains('sustainability') ||
      ...
      'daily_meal_plans': <Map<String, dynamic>>[
        <String, dynamic>{
          ...
          'dinner': <String, dynamic>{
            ...
            'total_calories': 520,
            'recipe':
                'Grill salmon fillet, roast sweet potato, steam broccoli.',
            'ingredients': <Map<String, dynamic>>[
              <String, dynamic>{
